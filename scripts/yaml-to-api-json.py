#!/usr/bin/env python3
"""Convert YAML subagent spec to JSON for the SRE Agent dataplane API.

The API expects an envelope: { name, type, tags, owner, properties: { ... } }
where properties contains camelCase fields matching the agent spec.
...
handoff_desc = spec.get("handoff_description", "")
if github_repo:
    instructions = instructions.replace("GITHUB_REPO_PLACEHOLDER", github_repo)
    handoff_desc = handoff_desc.replace("GITHUB_REPO_PLACEHOLDER", github_repo)

# Build the API envelope
api_body = {
    "name": spec["name"],
    "type": "ExtendedAgent",
    "tags": [],
